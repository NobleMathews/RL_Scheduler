...
session_types = ["_", "Max violation", "Max normalized violation", "__"]
num_cuts = range(10, 1160, 10)
        ...
        if os.path.exists(file_path):
            with open(file_path, "r") as f:
                try:
                    ...
                    if data.ndim == 1:
                        col_2_avg[i, j] = data[1]
                        col_3_avg[i, j] = data[2]
...
for session_type in session_types:
    ax1.plot(list(num_cuts), col_2_df.loc[session_type], label=f"{session_type} Departure")
    ax2.plot(list(num_cuts), col_3_df.loc[session_type], label=f"{session_type} Non Integ")
