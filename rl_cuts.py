...
if __name__ == "__main__":
    ...
    gamma = 0.99  # discount
    rrecord = []
    for session_type, _ in enumerate(["Lexicographic", "Max violation", "Max normalized violation", "Random cut selection"]):
        for e in range(1):
            ...
            while d != 0:
                ...
                if i % 100 == 0:
                    ...
                    if not os.path.isdir(f"bulk_session{session_type}_{i}"):
                        os.makedirs(f"bulk_session{session_type}_{i}")
                    np.save(f"bulk_session{session_type}_{i}/ab_{e}.npy", save_episode_np)
                    # append r to a file called reward_{i}.txt
                    with open(f"bulk_session{session_type}_{i}/reward.txt", "a") as f:
