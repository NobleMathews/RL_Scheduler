...
from es.policies import LinearRowFeaturePolicy, MLPRowFeaturePolicy, MLPRowFeatureAttenttionPolicy, \
    MLPRowFeatureAttenttionEmbeddingPolicy, MLPRowFeatureLSTMEmbeddingPolicy
...
def run(seed, num_time_steps, n_directions, step_size, delta_std, rollout_length, policy_type):
    # logdir
    logdir = 'esdata/es_shell/{}/seed_{}nd_{}stepsize_{}deltastd_{}policytype_{}'.format(name, seed, n_directions,
                                                                                         step_size, delta_std,
                                                                                         policy_type)
    ...
    while timestep_sofar <= num_time_steps:
        ...
        for i in range(n_directions):
            ...
            # rollout
            rewards, times = rollout_envs(envs=envs, policy=policy, num_rollouts=1, rollout_length=rollout_length,
                                          gamma=1.0)
        ...
        # eval rewards
        evaluated_rewards, _ = rollout_envs(envs=envs, policy=policy, num_rollouts=10, rollout_length=timelimit,
                                            gamma=1.0)
